...
def data_querying(input_text, history_text):
    ...
    template = """
        ...
        Extract Kibela link from provided context then include at the end of your answer with a two-line break.
...
def query_hyperdb(input_text, history_text):
    open_ai_embeddings = OpenAIEmbeddings(
        model=text_embeddings_model_name, chunk_size=chunk_size_limit
    )
    ...
    db = HyperDB(embedding_function=open_ai_embeddings)

    db.add_documents(documents)

    # Save the HyperDB instance to a file
    db.save("hyperdb/demo_hyperdb.pickle.gz")

    # Load the HyperDB instance from the save file
    db.load("hyperdb/demo_hyperdb.pickle.gz")

    # Query the HyperDB instance with a text input
    results = db.query(input_text, top_k=5)
    return results
